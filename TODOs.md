...
- [ ] **Cloudflare Pages project itself still needs to be created** — requires Oliver's Cloudflare login, which I can't do. Steps: Cloudflare dashboard → Workers & Pages → Create → Pages → Connect to Git → `AwsmOli/website`. Build command `yarn build`, output directory `dist`, no adapter needed (site is fully static). Custom domain (`blkpixel.com`) gets added after the first deploy, under the project's Custom Domains tab.
