...
- [x] Decided on **Cloudflare Pages** for deployment. Initialized git, excluded `CV.pdf` (has Oliver's exact home address/phone, more precise than what's shown on `/cv`) and `.claude/settings.local.json` via `.gitignore`, and pushed to a new public repo: https://github.com/AwsmOli/website. Added `.node-version` (pinned to 22.21.1) so Cloudflare's build picks the right Node runtime instead of defaulting to something older than the `>=22.12.0` engine requirement.
- [x] Cloudflare's current onboarding flow ("Create a Worker") deploys via Wrangler, not the classic Pages build+output-dir flow — added `wrangler.jsonc` (assets-only Worker pointing at `dist/`, verified with `wrangler deploy --dry-run`). Build command in the Cloudflare dashboard should be `npm run build` (not `yarn`, see below), deploy command `npx wrangler deploy` (prefilled default, correct as-is). Custom domain (`blkpixel.com`) gets added after the first successful deploy, under the project's settings.
- [x] First deploy attempts failed twice: (1) Cloudflare defaulted to Yarn 4 and tried to migrate the classic v1 lockfile, which its immutable-install rejected; (2) switched to npm entirely (see below) after discovering Yarn 1 doesn't reliably install sharp's platform-specific native binary on Cloudflare's Linux build container, even as a direct dependency. **The Cloudflare dashboard's "Build command" field still needs manually updating from `yarn run build` to `npm run build`** — Cloudflare auto-detects the install step from the lockfile, but that text field was typed in and won't rewrite itself.
- [x] Switched the whole project from Yarn Classic to **npm** (`package-lock.json`, `packageManager: "npm@10.9.4"` in `package.json`) — root cause of the sharp failure, not fixable by pinning a yarn version. Also fixed a latent `typescript@7` vs `@astrojs/check`'s peer range (`^5 || ^6`) mismatch that yarn only warned about but npm's stricter resolution rejected outright — downgraded to `typescript@^6.0.3`.
- [ ] **Cloudflare Web Analytics not wired up yet** — also needs Oliver's Cloudflare login to generate a beacon token (Cloudflare dashboard → Analytics & Logs → Web Analytics → Add a site → `blkpixel.com`). It's a separate product from Pages, decoupled from hosting. Once he has the token, add the beacon `<script>` to `BaseLayout.astro` and update `/datenschutz` to disclose it (cookieless, so no consent banner needed, but the privacy policy currently states no analytics is present and that needs correcting once this is live)
